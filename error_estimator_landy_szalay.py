...
rand2RA = rand2file['RA'][:]
rand2DEC = rand2file['DEC'][:]
...
#unq_all_healpixels_d2,hp_d2_cnts = np.unique(d2pix, return_counts=True)

#r2pix = hp.ang2pix(ns.nside, rand2RA, rand2DEC, nest=False, lonlat=True)
#unq_all_healpixels_r2,hp_r2_cnts = np.unique(r2pix, return_counts=True)
...
def make_sparse_mat(list,unq_all_healpixels_fn,pix,len_unq_hp,n,downgrade_vec):	
	...
	pl = []
	to_iterate_over = np.array(tuple(list[:,n]))[:,1]
...
for i in range(zbin):
	...
	Nd2 = len(data2RA[data2mask])
	Nr2 = len(rand2RA[rand2mask])
	print i, Nd2, Nr2
	...
	for j in range(zlen):
		...
		try:
			...
			pixel_lists = pickle.load(open('%s-%s-landy_szalay/%i_pix_list_0.p' % (truncate(ns.phot_name),truncate(ns.spec_name),name_ind),'rb'))

			inds = pixel_lists[0]
			inds_rand = pixel_lists[1]
			dd_pix_list = pixel_lists[2]
			dr_pix_list = pixel_lists[3]
			drspec_pix_list = pixel_lists[4]
			rr_pix_list = pixel_lists[5]
			...
			if np.any(inds):		
				...
				if flag == 0:
					...
					all_drspec_pix_list = drspec_pix_list
					all_rr_pix_list = rr_pix_list
					#print j
					allinds = inds
					allinds_rand = inds_rand
					...
					all_drspec_pix_list = np.concatenate((all_drspec_pix_list, drspec_pix_list))
					all_rr_pix_list = np.concatenate((all_rr_pix_list, rr_pix_list))
			
					#print j
					allinds = np.concatenate((allinds,inds))
					allinds_rand = np.concatenate((allinds_rand,inds_rand))
					print 'flag!=0', j, np.shape(allinds_rand), np.shape(all_rr_pix_list), '%s-%s/%i_pix_list.p' % (truncate(ns.phot_name),truncate(ns.spec_name),name_ind)
		...
		if flag2 == 0:
			...
			all_drspec_pix_list = np.array(all_drspec_pix_list)
			all_rr_pix_list = np.array(all_rr_pix_list)
	if flag != 0:
		print 'loaded files', time.time()-t0
		allinds = np.array(allinds).astype('int')
		allinds_rand = np.array(allinds_rand).astype('int')
		pix = hp.ang2pix(ns.nside,data2RA[allinds],data2DEC[allinds],lonlat=True)
		
		#print 5/0
		pix_rand = hp.ang2pix(ns.nside,rand2RA[allinds_rand],rand2DEC[allinds_rand],lonlat=True)
		...
		for n in range(np.shape(all_dd_pix_list)[1]):
			...
			pair_mat_drspec = make_sparse_mat(all_drspec_pix_list,unq_all_healpixels_fn,pix_rand,len_unq_hp,n,downgrade_vec)
			pair_mat_rr = make_sparse_mat(all_rr_pix_list,unq_all_healpixels_fn,pix_rand,len_unq_hp,n,downgrade_vec)
			...
			pair_mats_drspec.append(pair_mat_drspec)
			pair_mats_rr.append(pair_mat_rr)
		...
		drspec_data = np.array(map(lambda x: np.sum(x), pair_mats_drspec)).astype('float')
		rr_data = np.array(map(lambda x: np.sum(x), pair_mats_rr)).astype('float')
		...
		# Try the "naive" normalization here
		# the reasoning being that L-S is less sensitive to normalization errors
		wmeas = (dd_data/ (float(Nd1) * float(Nd2)) - dr_data/ (float(Nr1) * float(Nd2)) - drspec_data / (float(Nd1) * float(Nr2)) + rr_data / (float(Nr1) * float(Nr2)))/ (  rr_data / (float(Nr1) * float(Nr2)))
		#print 5/0
		...
		rand2RA_sel = rand2file['RA'][:][allinds_rand]
		rand2DEC_sel = rand2file['DEC'][:][allinds_rand]
		rand2Z_sel = rand2file['Z'][:][allinds_rand]		
		
		d2pix = hp.ang2pix(ns.nside, data2RA_sel, data2DEC_sel, nest=False, lonlat=True)
		unq_all_healpixels_d2,hp_d2_cnts = np.unique(d2pix,return_counts=True)

		r2pix = hp.ang2pix(ns.nside, rand2RA_sel, rand2DEC_sel, nest=False, lonlat=True)
		unq_all_healpixels_r2,hp_r2_cnts = np.unique(r2pix,return_counts=True)

		if len(hp_d2_cnts) != len(hp_cnts):
			new_hp_d2_cnts = np.zeros(len(hp_cnts))
			for i,hpp in enumerate(unq_all_healpixels):
				try:
					new_hp_d2_cnts[i] = hp_d2_cnts[unq_all_healpixels_d2 == hpp]
				except ValueError:
					continue
			hp_d2_cnts = np.copy(new_hp_d2_cnts)
		...
		if ns.jackknife:			
			if ns.equalize:		
				...
				loo_jackknife_drspec = np.zeros((nbins,len(hps)))
				loo_jackknife_rr = np.zeros((nbins,len(hps)))
				...
				for k in range(len(hps)):
					...
					Nd2 = np.shape(data2RA[data2mask])[0]
					Nr2 = np.shape(rand2RA[rand2mask])[0]
					...
					for l,hpk in enumerate(hps[k]):
						...
						Nd2 -=  hp_d2_cnts[np.where(unq_all_healpixels == hpk)]
						Nr2 -=  hp_r2_cnts[np.where(unq_all_healpixels == hpk)]
					...
					loo_jackknife_drspec[:,k] = resample(pair_mats_drspec,wts)
					loo_jackknife_rr[:,k] = resample(pair_mats_rr,wts)
					...
					wjack_loo[:,k] = ((loo_jackknife_dd[:,k])/ (float(Nd1) * float(Nd2)) - (loo_jackknife_dr[:,k])/(float(Nr1) * float(Nd2)) - (loo_jackknife_drspec[:,k])/(float(Nd1) * float(Nr2)) + (loo_jackknife_rr[:,k])/(float(Nr1) * float(Nr2))) /((loo_jackknife_rr[:,k])/(float(Nr1) * float(Nr2)))
					print loo_jackknife_dd[:,k],loo_jackknife_dr[:,k], wjack_loo[:,k]
			else:
				#print 5/0
				# Also deprecated in Landy-Szalay
			...
			l2o_jackknife_drspec = np.zeros((nbins,len(hps)*(len(hps)-1)/2))
			l2o_jackknife_rr = np.zeros((nbins,len(hps)*(len(hps)-1)/2))
			...
			for k in range(len(hps)):
				for l in range(len(hps)):
					if k < l:
						...
						l2o_jackknife_drspec[:,cnter] = resample(pair_mats_drspec,wts)
						l2o_jackknife_rr[:,cnter] = resample(pair_mats_rr,wts)
						Nd1 = np.shape(data1RA)[0]
						Nd2 = np.shape(data2RA[data2mask])[0]
						Nr2 = np.shape(rand2RA[rand2mask])[0]
						for hpk in hps[k]:
						...
						for hpl in hps[l]:
							...
							Nd2 -=  hp_d2_cnts[np.where(unq_all_healpixels == hpl)]
							Nr2 -=  hp_r2_cnts[np.where(unq_all_healpixels == hpl)]
						#Nd1 = float(np.sum(hp_d1_cnts[np.where(unq_all_healpixels == hps[k])]))
						Nr1 = np.shape(rand1RA)[0]-cnts[k]-cnts[l]
						print hps[k]
						print Nd1,Nr1
						wjack_l2o[:,k] = (l2o_jackknife_dd[:,k])/ (float(Nd1) * float(Nd2)) - (l2o_jackknife_dr[:,k])/(float(Nr1) * float(Nd2)) - (l2o_jackknife_drspec[:,k])/(float(Nd1) * float(Nr2)) + (l2o_jackknife_rr[:,k])/(float(Nr1) * float(Nr2))
			...
			def header(method):
				header_out='\n'.join([
						"SPEC=%s" % (ns.spec_name),
						"SPEC_RANDOM=%s" % (ns.spec_name_randoms),
						"z1=%g z2=%g zmean=%g" % (z1, z2, zmean),
						"PHOTO=%s" % (ns.phot_name),
						"PHOTO_RANDOM=%s" % (ns.phot_name_randoms),
						"N_SPEC=%d" % (data2mask.sum()),
						"N_SPEC_RANDOMS=%d" % (rand2mask.sum()),
						"N_PHOT=%d" % (np.shape(data1RA)[0]),
						"N_PHOT_RANDOM=%d" % (np.shape(rand1RA)[0]),
						"ERROR=%s" % (method),
			...
			base_out = np.array([theta, theta_low, theta_high, s, slow, shigh, wmeas])
