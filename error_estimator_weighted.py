...
cli.add_argument("--small_nside",default=16,type=int,help="nside for local wise density estimation")
...
data1_smallmap = np.bincount(data1_smallpix,minlength=12*small_nside**2,weights=wt1)
...
if np.max(d1pix) > np.max(all_healpixels):
	unq_all_healpixels = unq_all_healpixels_d1
...
if np.max(data1_smallpix) > np.max(rand1_smallpix):
	unq_all_healpixels_small = np.unique(data1_smallpix)
...
for i in range(zbin):
	...
	if flag != 0:
		...
		wmeas_no_adjust = dd_data/dr_data * float(Nr1)/float(Nd1) - 1.
		...
		if ns.jackknife:			
			if ns.equalize:		
				...
				for k in range(len(hps)):
					...
					for l,hpk in enumerate(hps[k]):
						...
						Nd1 -= np.sum(wt1[np.where(d1pix == hpk)])
			...
			base_out = np.array([theta, theta_low, theta_high, s, slow, shigh, wmeas, wmeas_no_adjust, wpoisson, w_mjb])
